...
## Reproduce Results
To reproduce the results presented throughout section IV, execute the Jupyter Notebooks where the loading, parsing, and analysis of the logs in this replication package were performed. The notebooks are available in the [data_analysis folder ](data_analysis/):

- [data_analysis.ipynb](data_analysis/data_analysis.ipynb): analysis and plots of the raw benchamrk measurements for latency and reliability.
- [prometheus_data.ipynb](data_analysis/prometheus_data.ipynb): analysis and plots of the recorded data in the monitoring subsystem during the benchmark execution.

The resulting figures generated by the notebook are in the [results folder ](results/).

Lastly, instrunctions on the installation and dependencies to run the Jupyter Notebooks are described below in section [Requirements and Dependencies
](requirements-and-dependencies).

In alternative you can visualize them in:

- [data_analysis.ipynb in NBViewer](https://nbviewer.org/github/kmolima/mqtt_benchmark/blob/main/data_analysis/data_analysis.ipynb)
...
The load is run concurrently with the help of the python script: [run_load.py](run_load.py)
...
The monitoring logs recorded in Prometheus are available in the [transformer/logs](transformer/logs) folder in compressed file which name starts with "monitoring" and has a suffix with the associated AUT and QoS level configured. 
